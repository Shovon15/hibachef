...
import downloadImage from "@/assets/images/menu-download.png";

const PerfectCelebration = () => {
  
  const handleDownload = async () => {
    if (!downloadImage?.src) return;

    try {
      const response = await fetch(downloadImage.src);
    ...
    <div className="relative  overflow-hidden">
      ...
      <ContentContainer className="relative z-20 bg-[#000000B2] h-full grid grid-cols-1 lg:grid-cols-8 items-center justify-center  px-6 text-white py-14 lg:py-20 gap-10 lg:gap-24">
        <div className="col-span-1 lg:col-span-5">
          ...
          <p className="mt-4 font-graphikTrial text-sm lg:text-base text-[#FFFFFF] leading-[150%] font-normal text-center lg:text-left">
        ...
        <div className="col-span-1 lg:col-span-3 space-y-4 lg:space-y-6">
          <button
            ...
            className=" px-6 py-3  text-white rounded-full bg-[#EE1F26] font-graphikTrial font-medium border border-[#EE1F26]   transition-colors duration-300 w-full text-sm lg:text-base uppercase"
          >
            Download Menu
          </button>
          <NavLink href={routes.partyEstimator}>
