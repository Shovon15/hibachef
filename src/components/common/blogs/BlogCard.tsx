...
const BlogCard = ({
  ...
  return (
    <div
      className={`grid ${
        isMainBlog ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"
      } ${isBgBlack ? "gap-4" : "gap-9"} justify-center items-center ${className}`}
    >
      {/* Main clickable area */}
      <NavLink href={routes.blog + "/dummy-details"}>
      ...
      <div>
        <h2
          className={`font-cooperBlack font-normal text-[clamp(1.5rem,1.875vw,2.25rem)] ${
            isBgBlack ? "text-[#FFFFFF]" : "text-[#121416]"
          }`}
        >
          Blog Title
        </h2>
        ...
        <div
          className={`flex justify-start items-center gap-3 font-graphikTrial text-lg mt-2 font-normal ${
            isBgBlack ? "text-[#FFFFFF]" : "text-[#6C757D]"
          }`}
        >
          <p>June 28, 2018</p>
        </div>
        ...
        <p
          className={`font-normal font-graphikTrial text-base mt-3 ${
            isBgBlack ? "text-[#FFFFFF]" : "text-[#6C757D]"
          }`}
        >
