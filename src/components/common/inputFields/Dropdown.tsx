...
export function Dropdown({
  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${className}`}>
      ...
      >
        <span className={isOpen || value ? `${inputTextColor}` : "text-gray-400"}>
          {value || placeholder}
        </span>
        {/* <ChevronDown
          size={20}
      ...
      {label && (
        <label
          className={`absolute origin-0 transition-all duration-300 pointer-events-none font-medium text-base font-graphikTrial ${
            isOpen || value
